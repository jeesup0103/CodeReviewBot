...
def get_pr_details(pr_url):
    headers = {
        'Authorization': f'token {os.getenv("REPO_GH_TOKEN")}',  # Using repository token for reading PR details
...
def get_code_changes(base_url, base_sha, head_sha):
    headers = {
        'Authorization': f'token {os.getenv("REPO_GH_TOKEN")}',  # Using repository token
...
def post_feedback_to_github(pr_url, feedback):
    ...
    response = requests.post(pr_comments_url, json={
        ...
        'Authorization': f'token {os.getenv("BOT_GH_TOKEN")}',
